...
| Số keypoint trung bình có v>0 mỗi người | 15,79 |
...
Đây cũng là các khớp khó gán nhất trong bộ ảnh, nhưng vì hai lý do khác nhau. Tai thường bị
...
**Những điểm đã sửa giữa hai lần chạy:**
...
## 3. Kiểm tra tính nhất quán cá nhân

Bài được thực hiện cá nhân nên không có visibility report của thành viên khác để đối chiếu.
Phần kiểm tra chéo với bạn cùng nhóm không áp dụng.

| Nội dung kiểm tra | Kết quả |
| --- | --- |
| Đủ ảnh và skeleton | 20 ảnh, 29 skeleton, đủ 17 keypoint mỗi skeleton |
| Kiểm tra định dạng | Đạt; `kpt_shape: [17, 3]` và mỗi dòng YOLO Pose có 56 số |
| Kiểm tra hình dáng | Đã xem ảnh từ `visualize_pose.py`, không còn lỗi đảo trái/phải được gold phân loại |
| Kiểm tra visibility | Sau rework: v=2 328, v=1 130, v=0 35 |
| Kiểm tra với gold | OKS trung bình 0,9224; OKS@0.75 1,0000; không còn skeleton cần rework |

Quy tắc được củng cố sau rework: nếu không nhìn thấy khớp nhưng vị trí giải phẫu vẫn nằm
trong biên ảnh thì phải đặt điểm ước lượng và dùng `v=1`; chỉ dùng `v=0` khi vị trí giải phẫu
thật sự vượt ra ngoài biên ảnh.
...
3. Ở `test_02`, tập nhãn chỉ có một người nhưng model báo hai người. Ảnh có một người dạng
   silhouette và hai con chim; dự đoán dư là lỗi **trượt hẳn**, vì model tạo một detection/pose
   trên vùng không phải người thay vì chỉ lệch nhẹ quanh khớp thật.
...
## 5. Một rule evidence đã sử dụng
